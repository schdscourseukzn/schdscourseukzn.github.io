...
murders_raw <- read_html(url) %>% html_nodes("table") %>% .[2] %>% 
               html_table %>% .[[1]] %>%
               setNames(c("state", "Gun_deaths", "Suicide", 
                          "Homicide", "Accident", "Law"))
head(murders_raw)
```

We realized that three of the columns that we expected to contain numbers actually contained characters:

```{r}
class(murders_raw$Gun_deaths)
class(murders_raw$Suicide)
class(murders_raw$Homicide)
```
...
So we've learned how to write 5 feet and 10 inches separately but what if we want to write them together to represent _5 feet and 10 inches_ like this `5'10"`. In this case neither the single or double quotes will work! This
...
In general, string processing involves a string and a pattern. In R we usually store strings in a character vector such as `murders$Gun_deaths`. Above we scrapped the web to obtain the `murders_raw` data. The first three strings in this vector defined by the gun deaths variable are:

```{r}
murders_raw$Gun_deaths[1:3]
```

Note that the usual coercion does not work here:

```{r}
as.numeric(murders_raw$Gun_deaths[1:3])
```
...
Before, we used code to scrape the web and create the `murders_raw` table. We noted that three (two that we care about) columns need to be parsed from characters into numbers but that commas where making it difficult. We can use the `str_detect` function to see that three of the six columns have commas in the entries:
...
```{r}
test_1 <- str_replace_all(murders_raw$Gun_deaths, ",", "")
...
```{r}
test_2 <- parse_number(murders_raw$Gun_deaths)
...
We still get NAs, but these are for truly missing values. This is actually due to the fact that the raw table we are using is for the year 2021, and not 2010 like the data available in `dslabs`.
...
```{r}
pattern <- ","
str_detect(murders_raw$Gun_deaths, pattern) 
```
...
and `str_view_all` shows us all the matches.
